...
def drawMatches(src_img, src_kp, dst_img, dst_kp, matches):
    draw_params = dict(
        matchColor=(0, 255, 0),  # draw matches in green color
        singlePointColor=None,
        # matchesMask=matchesMask[:44],  # draw only inliers
        flags=2,
    )
    return cv2.drawMatches(src_img, src_kp, dst_img, dst_kp,matches,None,flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)